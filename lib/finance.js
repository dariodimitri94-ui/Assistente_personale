...
// Scarica il file direttamente da Google Drive — l'indirizzo del file
// (condiviso come "chiunque abbia il link") funziona come una password,
// stesso principio dell'indirizzo iCal del calendario (Parte 5.2/5.8):
// niente account di servizio, solo un id da trattare con cura.
export async function scaricaDaGoogleDrive(fileId) {
  const url = `https://drive.usercontent.google.com/download?id=${fileId}&export=download&confirm=t`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Google Drive non raggiungibile (${res.status})`);
  const contentType = res.headers.get("content-type") || "";
  if (contentType.includes("text/html")) {
    throw new Error("Google Drive ha restituito una pagina invece del file — controlla che la condivisione sia \"chiunque abbia il link\"");
  }

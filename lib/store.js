...
// ============================================================
// Istantanee finanziarie (dentro il log giornaliero, Parte 5.8). Ogni
// estrazione ne produce una nuova; non si buttano, diventano lo storico.
// ============================================================
export async function saveFinanzeGiorno(data, snapshot) {
  return updateLogGiornaliero(data, { finanze: snapshot });
}

export async function getUltimeIstantaneeFinanze(limite = 30) {
  const rows = orThrow(
    await db()
      .from("log_giornalieri")
      .select("data, finanze")
  ...
  return rows.filter((r) => r.finanze && Object.keys(r.finanze).length > 0);
}

// ============================================================
// File del budget (Parte 5.8, adattata per Vercel — niente percorso
// locale, il file caricato dalla dashboard vive nel database)
// ============================================================
export async function saveFinanceFile({ filename, mimeType, contentBase64 }) {
  return orThrow(
    await db()
      .from("finance_file")
      .upsert(
        { user_id: USER_ID(), filename, mime_type: mimeType, content_base64: contentBase64, uploaded_at: new Date().toISOString() },
        { onConflict: "user_id" }
      )
      .select()
      .single()
...
export async function getFinanceFile() {
  return orThrow(
    await db().from("finance_file").select("*").eq("user_id", USER_ID()).maybeSingle()
  );
}
